...
namespace xc32{
	namespace sfr{
		struct i2c5:public sfr_register_mixin<i2c5>, public exclusive_mixin<i2c5>{
			static unsigned int i2c_no(){ return 5; }
		public:
			//Start Condition Enable bit : I2CxCONbits.SEN
			void start_enable(bool val_){I2C5CONbits.SEN=static_cast<unsigned char>(val_);}
			bool start_enable()const{return static_cast<bool>(I2C5CONbits.SEN);}
			//Restart Condition Enable bit : I2CxCONbits.RSEN
			void restart_enable(bool val_){I2C5CONbits.RSEN=static_cast<unsigned char>(val_);}
			bool restart_enable()const{return static_cast<bool>(I2C5CONbits.RSEN);}
			//Stop Condition bit : I2CxCONbits.PEN
			void stop_enable(bool val_){I2C5CONbits.PEN=static_cast<unsigned char>(val_);}
			bool stop_enable()const{return static_cast<bool>(I2C5CONbits.PEN);}
			//Receive Enable bit : I2CxCONbits.RCEN
			void receive_enable(bool val_){I2C5CONbits.RCEN=static_cast<unsigned char>(val_);}
			bool receive_enable()const{return static_cast<bool>(I2C5CONbits.RCEN);}
			//ACK Sequence Enable bit : I2CxCONbits.ACKEN
			void ack_enable(bool val_){I2C5CONbits.ACKEN=static_cast<unsigned char>(val_);}
			bool ack_enable()const{return static_cast<bool>(I2C5CONbits.ACKEN);}
			...
			void transmit_data(unsigned char val_){I2C5TRN=val_;}
			//ACK Status bti : I2CxSTATbits.ACKSTAT(0:ACK was received,1:ACK was not received)
			bool ack_status()const {return static_cast<bool>(I2C5STATbits.ACKSTAT);}
			//Receive Buffer Full Status bit : I2CxSTATbits.RBF
			bool receive_buf_full()const{return static_cast<bool>(I2C5STATbits.RBF);}
