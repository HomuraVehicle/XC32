#ifndef XC32LIB_SFR_TIMER1_INC
#define XC32LIB_SFR_TIMER1_INC 100
#
/*=====timer1=====*/
...
#	define XC32LIB_TIMER1_IRQ 4
#	define XC32LIB_TIMER1_VEC 4
#	define XC32LIB_TIMER1_IF IFS0bits.T1IF
#	define XC32LIB_TIMER1_IE IEC0bits.T1IE
#	define XC32LIB_TIMER1_IP IPC1bits.T1IP
#	define XC32LIB_TIMER1_IS IPC1bits.T1IS
#elif defined(XC32LIB_PIC32MZ)
#	define XC32LIB_TIMER1_IRQ 4
#	define XC32LIB_TIMER1_VEC _TIMER_1_VECTOR
#	define XC32LIB_TIMER1_IF IFS0bits.T1IF
#	define XC32LIB_TIMER1_IE IEC0bits.T1IE
#	define XC32LIB_TIMER1_IP IPC1bits.T1IP
#	define XC32LIB_TIMER1_IS IPC1bits.T1IS
...
#		define x_xc32_sfr_timer1_interrupt(void) __ISR(XC32LIB_TIMER1_VEC, XC32LIB_TIMER1_IPL_FOR_ISR) T1Interrupt(void)//*/func(void)
#	else
extern "C"{void x_xc32_sfr_timer1_interrupt(void); }
#	endif
#else
#	ifndef XC32LIB_DEBUGMODE
#		define xc32_sfr_timer1_interrupt(void) __ISR(XC32LIB_TIMER1_VEC, XC32LIB_TIMER1_IPL_FOR_ISR) T1Interrupt(void)//*/func(void)
#	else
...
namespace xc32{
	namespace sfr{
		...
		public:
			...
			void interrupt_flag(bool val_){XC32LIB_TIMER1_IF=static_cast<unsigned char>(val_);}
			bool interrupt_flag()const {return static_cast<bool>(XC32LIB_TIMER1_IF);}
			//interrupt enable bit : IECxbits.I2CxMIE
			void interrupt_enable(bool val_){XC32LIB_TIMER1_IE=static_cast<unsigned char>(val_);}
			bool interrupt_enable()const{return static_cast<bool>(XC32LIB_TIMER1_IE);}
			//Interrupt priority level : IPCxbits.I2CxIP
			void interrupt_priority_level(unsigned char val_){XC32LIB_TIMER1_IP=val_;}
			unsigned char interrupt_priority_level(){return XC32LIB_TIMER1_IP;}
			//Interrupt priority level : IPCxbits.I2CxIP
			void interrupt_sub_priority_level(unsigned char val_){XC32LIB_TIMER1_IS=val_;}
			unsigned char interrupt_sub_priority_level(){return XC32LIB_TIMER1_IS;}
...
//			void prescaler(timer::prescaler::div val_){T1CONbits.TCKPS=static_cast<unsigned char>(val_);}
			...
			void reset_all_config(){
				T1CON &= 0x00010000;			//ONˆÈŠO‚ð0‚ÅƒtƒBƒ‹
